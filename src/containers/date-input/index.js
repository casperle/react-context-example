...
export class DateInputComponent extends React.PureComponent {
  handleChange = date => {
    ...
    if (this.props.isDepartureDate && this.props.returnDate !== '' && moment(date).isAfter(this.props.returnDate)) {
      this.props.updateFlyDate(
        new Date(
          moment(date)
            .add(1, 'd')
            .toString()
...
export const DateInput = withConsumers(SearchConsumer)(DateInputComponent);
