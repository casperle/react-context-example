...
class LocationInputComponent extends React.PureComponent {
  ...
  loadLocations = debounce(async value => {
    ...
    try {
      ...
      const places = placesResponse.data.locations.map(place => ({
        id: place.id,
  ...
  getPopoverContent = () => {
    ...
    const $suggestionItems = this.state.locationSuggestions.map(location => (
      <div className={itemClass} onClick={this.createSuggestionClickHandler(location)} key={location.id}>
        {location.fullName}
      </div>
    ));
  ...
  render() {
    return (
      <Popover content={this.getPopoverContent()} isOpen={this.state.inputValue.length > 1 && this.state.isPopoverOpen}>
        <InputGroup
...
export const LocationInput = withConsumers(SearchConsumer)(LocationInputComponent);
